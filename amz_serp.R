...
# Import more User Agents -------------------------------------------------

#add source of the user_agents
url_source <-
  "https://raw.githubusercontent.com/cevaboyz/user_agents/main/db-2.csv"
#import the user_agents from the github repository
user_agent_list_git <- read.csv(url_source)
#transform the one column df to a single vector
user_agent_list_git_vector <- unlist(user_agent_list_git$USER_AGENT)


# SCRAPER -----------------------------------------------------------------

#take a random UA
ua <- sample(user_agent_list_git_vector, 1, replace = FALSE)

#get the SERP URL to scrape

url_to_scrape <-
  "https://www.amazon.it/s?k=asciugacapelli+remington&__mk_it_IT=%C3%85M%C3%85%C5%BD%C3%95%C3%91&crid=3HJD58BJSMJAS&sprefix=asciugacapelli+remington%2Caps%2C111&ref=nb_sb_noss_1"
...
# json_data <- scrape(url, content = "text/html; charset=UTF-8")
#
product_names <-
  scrape(url, content = "text/html") %>% html_nodes(".a-size-base-plus.a-color-base.a-text-normal") %>% html_text()


product_ratings <-
  parsed_html %>%  html_nodes(".a-size-base.puis-normal-weight-text") %>%
  html_text()

product_sold <-
  parsed_html %>% html_nodes(".a-size-base.a-color-secondary") %>% html_text()


filtered_product_sold <-
  product_sold[grep("\\d+\\+ acquistati nel mese scorso", product_sold)]
